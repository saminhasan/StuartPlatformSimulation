function obj = StuartPlatform(r, n, rB, dB, rP, dP)
    ...
    for i = 1:6
        ...
        obj.q_rots(i, :) = calculate_quaternion([1, 0 ,0]', v); % calculate the quat w.r.t to x frame.
    end
...
function angles = moveFunc(obj, pose)
    ...
    angles = asin(((vecnorm(l, 2, 2).^2) - ((obj.n^2 - 1)*obj.r^2))./ sqrt(ek.^2 + fk.^2)) - atan2(fk, ek);
end
