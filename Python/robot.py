from numpy import pi, cos, sin, sum, full, mean, sqrt, tile, array, zeros, arcsin, linalg, repeat, arctan2, radians, zeros_like, set_printoptions
...
class StuartPlatform:
    def __init__(self, r, n, rB, dB, rP, dP):
        ...
        self.B = array([rB*cos(self.gammaB) + dB*cos(self.betaB), rB*sin(self.gammaB) + dB*sin(self.betaB), zeros_like(self.gammaB)]).T
        self.H = array([(rotZ(beta_k) @ [self.r, 0, 0]) + B_k for beta_k, B_k in zip(self.betaB, self.B)])
        self.P = array([rP*cos(self.gammaP) + dP*cos(self.betaP), rP*sin(self.gammaP) + dP*sin(self.betaP), zeros_like(self.gammaP)]).T
    ...
    def move(self, pose):
        ...
        theta_i = arcsin( (linalg.norm(l, axis=1)**2 - ((self.n**2 -1)*self.r**2))/sqrt(ek**2 + fk**2) )- arctan2(fk, ek)
        return theta_i, P


    # def h(self, alpha=zeros((6))):
    #     return  array([rotZ(beta_k) @ rotY(-alpha_k) @ [self.r, 0, 0] + B_k for alpha_k, beta_k, B_k in zip(alpha, self.betaB, self.B)])
