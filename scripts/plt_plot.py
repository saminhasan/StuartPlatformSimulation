...
if __name__ == "__main__":
    ...
    poly = Poly3DCollection([B], alpha=0.25, facecolor="k")
    ...
    for i in range(len(rb_origins)):
        ...
        ax.text(mid_point[0], mid_point[1], mid_point[2] + 0.005, "rP", color="k", fontsize=10, ha="center", va="center")
    ax.quiver(
        db_origins[:, 0],
        db_origins[:, 1],
        db_origins[:, 2],
    ...
    for i in range(len(db_origins)):
        ...
        ax.text(mid_point[0], mid_point[1], mid_point[2] + 0.005, "dP", color="k", fontsize=10, ha="center", va="center")
    ...
    ax.text(O_b[0] + L * 1.05, O_b[1], O_b[2], "$P_x$", fontsize=14, color="r")
    ax.text(O_b[0], O_b[1] + L * 1.05, O_b[2], "$P_y$", fontsize=14, color="g")
    ax.text(O_b[0], O_b[1], O_b[2] + L * 1.05, "$P_z$", fontsize=14, color="b")
    ax.text(O_b[0], O_b[1], O_b[2] - 0.02, "$O_P$", fontsize=14, color="k")
    for k in range(6):
        c = B[k]  # center at B_k
        ...
        ax.text(c[0], c[1], c[2] - 0.02, f"$P_{{{k+1}}}$", fontsize=14, color="k")
